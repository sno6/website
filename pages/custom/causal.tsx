...
export default function Causal() {
    return (
        <div>
            ...
            <p style={{ fontSize: 30 }}>
                h<span style={{ color: "#BCCCFF" }}>⁰ </span>
                e<span style={{ color: "#BCCCFF" }}>¹ </span>
                l<span style={{ color: "#BCCCFF" }}>²	</span>
                l<span style={{ color: "#BCCCFF" }}>³	</span>
                o<span style={{ color: "#BCCCFF" }}>⁴	</span>
            </p>
            ...
            <p>What's illustrated here is that higher timestamps represent operations that happened with more context, or at a later point in the document's history, and as such they are traversed first when building the final value.</p>
            <p>So what did the user type and in what order to create the tree above?</p>
            <ul>
                <li>They first typed “Causatrees”</li>
                <li>Realising they misspelled the word they added an "l " after "Causa"</li>
            </ul>

            <p>Notice how there are two branches after the 6th node? it makes sense we first traverse the branch with the higher timestamp because it was added after the misspelling had occurred. But there's a few more cases we need to be aware of.</p>

            <h2>Traversal</h2>

            <p>Which brings us to an important topic, tree traversal and sibling trees. To produce the correct final value we need to traverse the tree in depth-first pre-order, but with special cases for sibling branches. Those special cases are:</p>
            <ul>
                <li>If a node has multiple children (sibling branches) traverse the branches ordered by timestamp descending order. We saw this in the previous example.</li>
                <li>If branches have the same timestamp, traverse the branch that has the higher entity id first. What you order by here isn’t important but it is critical that we order in a way that is consistent. Ordering by entity id is just an example of that.</li>
            </ul>

            <p>I said at the start of the article that causal trees can be used in collaborative, networked environments, which means that it needs to handle concurrent updates, duplicate writes, partitioning between clients and all of the stuff we love to think about when it comes to distributed system design. So how does it do that?</p>

            <p>When some tree (a) merges with another tree (b), the merge operation is essentially a diff and patch. What that means is we find all of the nodes from tree ‘a’ that aren’t in ‘b’ and add them to ‘a’. This operation is idempotent, which means we can perform it any number of times and the result will be the same, like inserting a new value to a set. Two other properties that are required for a CvRDT are commutativity and associativity. Commutativity ensures that the order of merging does not affect the final result, for instance, merging tree 'a' with 'b' yields the same result as merging 'b' with 'a'. Associativity allows for grouping merge operations in any order, so combining 'a' with 'b', and then with 'c', gives the same result as merging 'a' with the result of merging 'b' with 'c'. When each tree (or node) in a distributed system observes the same set of changes, even if in different orders, they will eventually converge to the same consistent state due to these properties. Let’s look at an example.</p>
            ...
            <p>This demonstration showcases how causal trees can resolve concurrent updates in a consistent manner. Whether you would want to do that instead of showing conflicts to the user for them to resolve themselves is up to the application.</p>

            <h2>Deletion</h2>

            <p>You may have noticed by now that if you delete a node, the value of that node changes to a 🪦. When a node is deleted we need to keep it around for consistency reasons, but the value is ignored when we create the final value from traversing the tree. This is a common technique found in distributed database design and CRDT designs known as tombstoning. I’ll leave it as an exercise to the reader to try to understand how consistency would be impacted if we removed the node completely from the tree on deletion.</p>
